...
class CombinedFollower(Node):
    ...
    def scan_cb(self, scan: LaserScan):
        ...
        # Only check primary AVD rectangle if not in START, not already in AVD, and moving forward enough:
        if self.obs_status not in ("START",) and self.coll_avd != "AVD" and self.prev_speed > 0.2:
            ...
            if ps <= vmin:
                ...
                # linear interp between 0.14 and 0.22
            ...
            for r, ang in zip(ranges, angs):
                if not (math.isfinite(r) and r >= 0.04):
                    continue
                x = r * math.cos(ang)
                # In certain GREEN_CLOSE/RED_CLOSE and section0 mod 4, tighter x_max
            ...
            if valid_pts >= 10:
                ...
                try:
                    ...
                    pass
                # If reached 12 consecutive scans, set AVD mode and adjust heading
                if self.coll_count >= 12 and self.coll_avd != "AVD":
                    # Compute average y over these valid points of this scan (approx)
                    avg_y = sum_y / valid_pts if valid_pts > 0 else 0.0
                    # Adjust target_heading by ±1° based on avg_y
                    if avg_y > 0.01:
                        self.target_heading -= 2.2
                        self.get_logger().info(f"AVD adjustment: avg_y={avg_y:.3f}>0 → subtract 1° → new target_heading={self.target_heading:.2f}")
                    elif avg_y < -0.01:
                        self.target_heading += 2.2
                        self.get_logger().info(f"AVD adjustment: avg_y={avg_y:.3f}<0 → add 1° → new target_heading={self.target_heading:.2f}")
                    else:
                ...
                # reset consecutive primary count
                self.coll_count = 0
