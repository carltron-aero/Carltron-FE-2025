...
class CombinedFollower(Node):
    def __init__(self):
        ...
        self.exposure_time_absolute = 50
        self.gain = 100
    ...
    def scan_cb(self, scan: LaserScan):
        ...
        # --- Prepare LIDAR sampling helpers ---
        ...
        # Detection & selection only if mode == 'OBS' and frame available
        if self.mode == 'OBS' and (self.obs_status == "SEARCH" or self.obs_status == "DRIVE") and display_frame is not None:
            ...
            # Compute side distance once, rotated sample_range
            ...
            for c in clusters:
                ...
                # Compute raw perpendicular and new_perp
            ...
            # SEARCH -> DRIVE after enough scans (always run)
            if self.obs_status == "SEARCH":
                print("Q " + str(self.search_count))
        ...
        # Collision avoidance override turned off
        self.coll_avd = "N"

        # Control logic blocks
        speed = 0.0; steer = 0.0; m = self.mode

        # Mode transitions using synced yaw_deg
        if self.direction == "CLOCK" and (yaw_synced_deg < -1070):
            self.donetimes += 1
            if self.donetimes >= 6:
                m = "Done"
        elif self.direction == "COUNTER" and (yaw_synced_deg > 1065):
            self.donetimes += 1
            if self.donetimes >= 8:
                m = "Done"
        ...
        if m == "W_CLCW":
            ...
            if d_front >= 0.9:
                ...
                # at d_front=0→speed_min; at d_front=0.6→speed_max
            ...
            if d_front < 0.8 and abs(steer_nominal) < 0.22 and selected == None:
                speed = -0.42
