...
class CombinedFollower(Node):
    def __init__(self):
        ...
        # Collision avoidance (unused for now)
        self.coll_avd = "N"
        self._avd_start = 0.0
        ...
        try:
            if self.auto_exposure:
                ...
                subprocess.run(
                    ...
                    check=True)
                # gain or white_balance if needed:
                # subprocess.run(["v4l2-ctl", f"--device={dev}", f"--set-ctrl=gain={self.gain}"], check=True)
                # subprocess.run(["v4l2-ctl", f"--device={dev}", f"--set-ctrl=white_balance_temperature={self.white_balance_temperature}"], check=True)
        except Exception as e:
        ...
        # WAIT mode
            ...
            if abs(lateral_error) > 0.07:
                ...
                if ((d_dir == "CLOCK" and d_mode == "GREEN_CLOSE") or (d_dir == "COUNTER" and d_mode == "RED_CLOSE")) and self.section_count % 4 == 0:
                    steer_nominal = 0.039 * heading_error + 5.8 * lateral_error
                else:
            ...
            #print("le_ " + str(lateral_error) + "he_ " + str(heading_error) + "sn_ " + str(steer_nominal))
            ...
            speed = -0.22; steer = 0.0
        else:
            speed = 0.0; steer = 0.0

        if self.mode == "OBS" and self.section_count == 12:
            # measuring distance clamp for last section
