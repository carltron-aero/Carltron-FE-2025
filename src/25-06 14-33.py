...
class CombinedFollower(Node):
    def __init__(self):
        ...
        self.kp_heading = 1.0
        self.kp_lateral = 1.0
        self.angle_limit_corr_gain = 0.55 #previously 0.55
    ...
    def scan_cb(self, scan: LaserScan):
        ...
        if self.obs_status == "START":
            ...
            if d_front < 1.36 and abs(lateral_error) < 0.07:
                speed = -0.5
                if self.stop_times < 5:
                    self.stop_times += 1
                else:
            ...
            d_front = sample_range(start_f, end_f)
            if d_front >= 0.4:
                speed_front = speed = 0.54
            elif d_front <= 0.2:
                speed_front = self.speed_min
            else:
                frac2 = (d_front - 0.2) / (0.6- 0.0)
                speed_front = self.speed_min + (0.54 - self.speed_min) * frac2
        ...
        if self.mode == "OBS" and self.section_count == 0 and (not self.park_active):
            ...
            if 0.5 < (d_lb + d_rb) < 1.05:
                ...
                if self.lapstop_counter <= 11:
                    ...
                    elif time.time() - self.stop_time >= 8.0:
        ...
        if stop_turn:
            ...
            if self.stop_times < 5:
                ...
                if self.obs_status == "PARK_TURN":
                    ...
                    self.get_logger().info(f"TURN complete: set obs_status=TURN, theoretical target_heading={self.target_heading:.1f}°")
                else:
